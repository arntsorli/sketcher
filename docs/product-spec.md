...
A non-destructive clipping plane supports X/Y/Z normals, millimetre position, direction flip, a visible helper, and reset without changing authoritative geometry.
...
The terrain overlay searches Norwegian places, previews matching Esri satellite or topographic imagery, and selects up to a 2×2 km visible rectangle or polygon. The extraction preserves aspect ratio, supports up to a 4096-pixel maximum dimension and targets up to 8 pixels per metre, then clips the cached image to the chosen polygon on a flat Z=0 surface. The selected source, WGS84 bounds, attribution, image, and local dimensions are embedded for offline reopening.
